...
In TL/POJ, the deterministic subset now converts in both preedit and commit: every valid syllable has a numeric tone, so each digit proves the next boundary (`lo5ma2ji7` → `lô-má-jī`); invalid and partial-tone chains remain verbatim. The remaining design work is limited to ambiguous/toneless input and any Enter-only policy; raw output remains available through the existing strip-#0 literal candidate. No platform-specific conversion is needed because `phonetics::normalize_tone` is shared by every composing surface.
