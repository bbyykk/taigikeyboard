...
**Status**: Design LOCKED; deterministic fully-toned display slice implemented in the current working tree on 2026-09-15 after USER confirmation. The remaining Enter-only behavior for ambiguous/toneless input is not implemented. Original plan was pre-arranged 2026-06-29 at USER request to minimize impl-time effort (USER 「預先安排好v3.6.5的項目，減少之後實作的effort」; originally tagged v3.6.4 2026-06-20, moved because i18n took v3.6.4). **Full design + code seams + Codex prompt**: memory `project_roman_convert_on_commit.md`.

In TL/POJ, Enter should commit the **converted** romanization — multi-syllable segmentation + numeric tone → tone-diacritic (`suann2ting3` → `suán-tìng`), matching PhahTaigi. The deterministic subset now converts in both preedit and commit: every valid syllable has a numeric tone, so each digit proves the next boundary (`lo5ma2ji7` → `lô-má-jī`); invalid and partial-tone chains remain verbatim. Requester = Kisaragi Hiu (same person who drove the S22/§34 literal-roman candidate). Remaining design work is limited to ambiguous/toneless input and any Enter-only policy; raw output remains available through the existing strip-#0 literal candidate. No platform-specific conversion is needed because `phonetics::normalize_tone` is shared by every composing surface.
