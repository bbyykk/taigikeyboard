...
- Transformations **NOT** applied: word-boundary inference (no spaces), candidate matching, ranking, or ambiguous syllable segmentation. A fully-toned numeric chain is the narrow exception: each digit proves a boundary, so `lo5ma2ji7` renders as `lô-má-jī` without consulting candidates or ranking.
...
**Ambiguous engine-driven auto-hyphenation remains out of scope.** `Intent::AppendHyphen` (api.rs) still treats `-` as a user-typed character. The display layer now handles only the deterministic fully-toned grammar `(valid-syllable + tone-digit){2,}`: `goa2ai3li2` → `goá-ài-lí`. Toneless `goaaili`, partial-tone `goa2aili`, and invalid `xyz2abc3` remain verbatim; choosing boundaries for them would require a syllabifier/lattice policy and could make the preedit jump with ranking. **Distinct from the *Compound-hyphen* contract above** (2026-05-18): that inserts a `-` between two already-**nailed** single-syllable segments whose dictionary identity is *known*.
