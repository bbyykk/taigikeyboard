...
- **輸出 is a row on the 一般 pane, bound to the stored swap.** Labelled with the 快捷鍵 pane's own `desktop.shortcutSectionOutput` (輸出 — the same thing in the same word, no second key). macOS `GeneralSettingsView` `@AppStorage(Keys.isTranslateSwapped)` picker, `漢字` = `true`, `羅馬字` = `false`; Windows `general.rs` `choice_row` over `OUTPUT_SCRIPTS = [true, false]` writing `Message::SetSwitch(IS_TRANSLATE_SWAPPED, …)`. Same key the `` ` `` chord flips (`TaigiInputController.performShortcutAction .toggleTranslateSwapped`, `session.rs ShortcutAction::ToggleTranslateSwapped`), so the chord and the pop-up are one setting seen from two places — a chord press repaints the open pane through the binding / the live-reload beat.
...
**Scope**: all four platforms for the default; macOS + Windows for the pane. No engine change, no `make build`. i18n: `settings.outputScriptHanji` / `outputScriptRoman` (desktop scope); the label reuses `desktop.shortcutSectionOutput`.
