...
`dictionary/common/abbrev.py` gives every ≥ 2-syllable row an initial-letter face — `tl_abbrev` (`só-sî` → `ss`), `poj_abbrev` (over the POJ display, `chia̍h-pn̄g` → `cp`), `tps_abbrev` (`ㄙㄒ`) — and `create_fst.py` indexes it in the same FST family as the toneless key. The keyboard reached those rows through the normal-mode `lexicon::search` exact++prefix merge until v3.5.8 made the candidate fetch continuous-only; from then on every continuous fetch rejected acronym hits on purpose (#297 `matches_continuous_tl_toneless_key`, #443 `skip_abbrev`), because a **span** key `tl:gi` inside `ginalangtsiahpngbesai` is a syllable and 外夷 `guā-î` shares no letters with it. The custom dictionary kept its `form IN (?, 'abbrev')` match, so a user's own `só-sî` answered `ss` while the dictionary's did not (USER 2026-09-18 「input "ss", should return "鎖匙só-sî"」).

- **Gate = the buffer's shape, not the fetch branch.** `composing::shadow::abbrev_query_key` emits `<family>:<lowercased raw>` only when the whole buffer is acronym-shaped for the mode — TL / POJ `phonetics::is_roman_acronym_key` (every char an ASCII consonant, the string does not parse as a syllable chain: `ss` / `tk` / `mk` / `ts` yes; `ng` / `m` / `tngtng` / `mngkng` / `si` / `ai` no), TPS `phonetics::is_tps_initial_only`, plus ≥ 2 glyphs. Both predicates require every char to be such a glyph, so a digit, tone mark, hyphen or space disqualifies the buffer: no `TonePin`, no barrier, `s-s` / `ss2` / `ㄙㄒˊ` stay on the syllable paths. The step (4c in `assemble_candidates`) runs after BOTH the empty-keys and the span-local branch — `mk` has the left-anchored syllable `m` and is still the abbreviation of 物件.
- **A hit is accepted only against the record's own face.** `lexicon::fetch_abbrev_candidates` does an EXACT `lookup_exact` of the key and keeps a row only when `abbrev_face_eq` holds: `phonetics::derive_abbrev(record.tl)` (`tl:`), the same over `tl_display_to_poj_display(record.tl)` (`poj:`), `tps_abbrev_from_tl` or its C-3a er→or variant (`tps:`). Exact, not prefix: `ss` does not reach 總鎖匙 (`tss`). The span-local / walker / partial-prefix guards are untouched — `gi` and `ginalangtsiahpngbesai` still never surface 外夷.
...
- **The gate does not backtrack.** `is_roman_acronym_key` now runs on the whole buffer every keystroke, and its syllable-chain check was a backtracking scan — exponential on `mngmng…s` (each `mng` splits two ways, the tail `s` fails every branch; 16 repeats > 8 s, Codex post-impl 2026-09-18 P1). `phonetics::syllable::splits_into_syllables` is a forward O(n²) DP over byte offsets with the same verdict; `abbrev_query_key` also rejects on length / mode / a per-char consonant screen before it allocates or calls the predicate.
- **Metadata is the record's.** `consumed_span` = the whole buffer (an abbreviation commits everything typed), `syllable_count` = the record's (鎖匙 = 2 — one typed glyph per syllable, not one syllable typed), `canonical_tl` = the record's TL, so frequency / NextWord learn the same `(漢字, canonical-TL)` identity as a syllabic selection. Recase mirrors the buffer (`SS` → `SÓ-SÎ`); POJ mode renders POJ (`cp` → `chia̍h-pn̄g`). Custom entries are not merged again (the fetch before already merged them for the same buffer).
...
- **Vowel-initial abbreviations stay out.** `ai` (阿姨) is not acronym-shaped and the syllabic reading 愛 keeps the strip — the same conservative cut both predicates already made for the partial-prefix skip. Widening to v3.4.2's "any 2-letter key" behaviour is a product decision, not a bug.
- **Direction, not this slice (Core Principle #5).** The shape gate and the face check both exist because the FST cannot tell an abbrev key from a toneless key in the same family (the code has said so since #443: `phonetics/src/syllable.rs` / `tps.rs` "fully separating the abbrev family needs an FST family tag"); the custom dictionary already carries that tag (`search_key.form`). An abbrev family tag in `create_fst.py` (+ `make dict`, FST refresh on four platforms, `search.rs` union for Tab3) would retire `skip_abbrev`, `is_tps_acronym_face_hit`, the shape predicate and the face check, and would make vowel-initial abbreviations reachable — a product call for the USER. Also visible: the custom-dictionary abbrev match is `LIKE ? || '%'` (prefix) on all four platforms while this dictionary lookup is exact; pre-existing platform SQL, not changed here.

**Scope**: shared Rust engine (`phonetics::derive_abbrev` made public, `lexicon::fetch_abbrev_candidates` + `COVERAGE_KIND_ABBREV`, `composing::shadow::abbrev_query_key` + Step 4c). Needs `make build`; no `make dict`, no platform code — all four platforms inherit through `FetchAtPos`.

**Tests**: `phonetics/src/syllable.rs` (`is_roman_acronym_key_does_not_backtrack_on_ambiguous_nasal_chains` — 16 × `mng` + `s` under 100 ms), `composing/src/shadow.rs` (`abbrev_query_key_*` — the gate incl. `mk` / `ngs` / `ts` accepted, `ng` / `m` / `tngtng` / `mngkng` / `s` / `ss2` / `s-s` / `ai` / TPS tone-mark / space rejected), `lexicon/tests/whole_buffer_abbrev.rs` (exact face match per family, index-drift rejection, source filter, user-weight precedence, sorted un-truncated pool), `lexicon/tests/roman_num_face_parity.rs` (`tl_abbrev` / `poj_abbrev` columns byte-match `derive_abbrev` / `poj_abbrev_from_tl` over all 143 704 multi-syllable CSV rows), `composing/tests/continuous_abbrev.rs` (end-to-end `Start → EnterContinuous → FetchAtPos` against `common::build_dictionary_fst`, which now emits the abbrev keys: literal at 0, block order under `ts`, span-local-branch `mk` with seven partial/abbrev overlaps deduped and the block still filled to the cap, recase, separator / digit / single-glyph negatives, POJ face + render, TPS byte span, custom-entry dedupe). Production `candidate_dump`: `ss` → 先生 … 鎖匙 [9]; `tk` → 逐家 [1] / 大家 [4]; `ts` → 這/做/早 … unchanged then 正實 … at [31]; `mk` → 毋 … `m̄-k…` extensions … then 物件 [36], 66 candidates; `gi` / `ginalangtsiahpngbesai` no 外夷; POJ `cp` → 食飯; TPS `ㄙㄒ` → 鎖匙 [1], `ㄇ` / `ㄇㄒ` / `ㄇㄒㄧ` / `ㄎㄛㆻㄫ` / `ㄍㄠ` unchanged; 16 × `mng` + `s` answers immediately. Cross-platform device acceptance: **S50** (`docs/architecture/dogfood-checklist.md`).
