...
- **Composing display** (`convert_syllable`): places the tone mark **directly on the typed literal syllable** via `tl::apply_tl_tone_literal` / `poj::apply_poj_tone_literal` — no spelling fold. The mode only selects the tone-mark glyph + vowel-priority convention. A token is validated as a single syllable (`split_initial_final ∘ normalize_to_tl`, recognizes `goa`/`chiah`/`ere`/`iri`). A valid fully-toned numeric chain has deterministic boundaries at its digits and is split before conversion (`lo5ma2ji7` → `lô-má-jī`); toneless, partial-tone, and invalid chains stay verbatim because their boundaries are not proven.
...
- **WYSIWYG**: the candidate `roman == display_text ==` the preedit literal (`composing::derived::derived_display`) **exactly**. It is the literal typed spelling with tone marks only — NO spelling fold (`INVARIANT_TL_INPUT_LITERAL_NO_POJ_SPELLING_FOLD` §30). It is NOT re-run through the Step 5 POJ recase (`derived_display` is already the mode-correct POJ/TL literal). A valid fully-toned numeric chain uses each digit as a deterministic syllable boundary (`lo5ma2ji7` → `lô-má-jī`); toneless, partial-tone, and invalid chains remain verbatim. The candidate always matches the underline.
